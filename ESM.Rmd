...
1. Has developed land cover increased over time in the known habitats of species 2 and 3?
...
3. Are these land cover changes different from rates of change throughout Texas?

4. Across Texas, are increases in developed land cover over time higher in catchments/watersheds with higher levels of BFI?
...
Eurycea <-
  ...
  mutate(BFI = BFICAT) %>% # We're working at the catchment level
...
Eurycea.only <- # Dataframe of only salamander presences
  Eurycea %>%
  filter(Species != "background") # Remove all background catchments

Eurycea.unnamed <- # Dataframe of only undescribed species
  Eurycea %>%
...
ggplot(Eurycea.unnamed, aes(x = Year, 
                         y = PercentDevelop, 
                         color = Species)) + 
                     geom_point() + 
  # Add line of best fit for each species
...
This first plot shows a clear upward trend in development within catchments from 2001 to 2019 for most Eurycea species. The slope of this trend is more extreme for some species, especially E. neotenes.
...
## Question 1: Has developed land cover increased over time in the known habitats of species 2 and 3?

```{r development change over time}

devtime.lm <- lm(data = Eurycea.unnamed, PercentDevelop ~ Year)
summary(devtime.lm) # Fails to achieve significance
...
## Question 2: Between species 2 and species 3, which species' habitat has a higher percentage and/or rate of increase for developed land cover?

```{r compare 2 and 3}

Eurycea.sp2sp3 <- # Dataframe of only sp. 2 & sp. 3
  Eurycea.unnamed %>%
  filter(Species %in% c("sp2","sp3"))

devsp.lm <- lm(data = Eurycea, PercentDevelop ~ Species)
summary(devsp.lm) # sp. 3 achieves signifance

```

## Question 3: Are these land cover changes different from rates of change throughout Texas?

```{r development change over time}

devtime.lm <- lm(data = Eurycea, PercentDevelop ~ Year)
summary(devtime.lm) # Achieves significance, but very low R-squared

ggplot(Eurycea, aes(x = Year, 
                    y = PercentDevelop)) + 
  stat_smooth(method = "lm", 
              formula = y ~ x, 
              se = TRUE,
...
## Question 4: Across Texas, are increases in developed land cover over time higher in catchments/watersheds with higher levels of BFI?
...
This simple linear model suggests that base flow index and development cover are negatively correlated. The model achieves significance, but has an adjusted R-squared of less than 1%. 
